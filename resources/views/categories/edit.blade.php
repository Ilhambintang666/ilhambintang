...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h3><i class="fas fa-edit me-2 text-danger"></i>Edit Kategori</h3>
        <p class="text-muted mb-0 small">Perbarui informasi kategori: <strong>{{ $category->name }}</strong></p>
    </div>
                ...
                <form action="{{ route('categories.update', $category) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control premium-input @error('description') is-invalid @enderror"
                                  ...
                                  name="description"
                                  rows="4"
                                  placeholder="Deskripsi kategori barang (opsional)">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text mt-2"><i class="fas fa-info-circle me-1"></i> Berikan deskripsi singkat tentang kategori ini.</div>
                    </div>

                    <div class="alert alert-info border-0 shadow-sm rounded-3 mb-5" style="background:rgba(13,202,240,0.05); border-left: 4px solid #0dcaf0 !important;">
                        <i class="fas fa-info-circle me-2 text-info"></i>
                        Kategori ini memiliki <strong class="fs-5">{{ $category->items->count() }}</strong> barang terdaftar.
                    </div>
